...
// - Budget-aware composer that prioritizes must-haves, then cheapest items
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 2) Build candidates from spec (every product query wrapped with cache)
    const candidates = await getCandidatesFromSpec(spec);

    // 3) Compose the bundle under budget
    const bundle = composeBundle({ spec, candidates });

    return res.status(200).json(bundle);
  } catch (err) {
    console.error('API error:', err);
    // Final fallback so UI still shows something usable
    const items = [
...
/** ---------------- Step 2: Candidates from spec (wrap search with cache) ---------------- */
...
/** ---------------- Phase A: searchProducts() with RapidAPI primary + optional Apify fallback ---------------- */
async function searchProducts({ query, limit = 3 }) {
  // 1) RapidAPI Google Shopping (primary)
...
async function rapidApiGoogleShopping({ query, limit }) {
  ...
  // Adjust path/params per the provider you subscribed to on RapidAPI.
  const url = new URL(`https://${RAPIDAPI_HOST}/search`);
  url.searchParams.set('q', query);
  url.searchParams.set('gl', 'US');
  url.searchParams.set('hl', 'en');
  url.searchParams.set('num', String(Math.max(3, limit))); // if provider supports
  ...
  // Providers vary: inspect their "Endpoints" example and tweak these fields.
...
/** Apify generic search (actor-based; adjust input per the actor’s docs) */
